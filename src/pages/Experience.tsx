  ...
  return (
    <div className="relative px-6 py-8 md:px-10 md:py-9">
      ...
      <div className="mt-4 flex flex-wrap items-center gap-x-5 gap-y-1.5 text-[11px] font-semibold uppercase tracking-[0.2em] text-black">
        {entry.period && (
          <span className="flex items-center gap-2">
            <span
              aria-hidden="true"
              className="h-2 w-2 rotate-45 bg-[#B80404]"
            />
            {entry.period}
          </span>
        )}
      ...
      {entry.highlights.length > 0 && (
        <ul className="mt-5 space-y-2.5 border-l-[3px] border-black pl-4">
          {entry.highlights.map((highlight) => (
            <li
              key={highlight}
              className="flex gap-2.5 text-[13px] leading-snug text-black"
            >
              <span
                aria-hidden="true"
                className="mt-[6px] h-2 w-2 shrink-0 rotate-45 bg-[#B80404]"
              />
              {highlight}
            </li>
          ))}
        ...
        <div className="mt-5 flex flex-wrap gap-2">
          {entry.tech.map((tech) => (
            ...
            >
              <span
                aria-hidden="true"
                className="h-1.5 w-1.5 rotate-45 bg-[#B80404]"
              />
              {tech}
            </span>
          ))}
      ...
      <div className="mt-6 flex items-center justify-between border-t border-black pt-3 text-[10px] font-bold uppercase tracking-[0.25em] text-black">
        <span>
          Mensaje {nn} / {String(TOTAL).padStart(2, "0")}
        </span>
        <span className="flex items-center gap-2">
          <span>Leido</span>
          <ReadChecks />
      ...
      >
        ...
        >
          <div className="relative">
            ...
            <div
              data-cursor="select"
              className="relative bg-black p-[6px] md:p-[7px]"
              style={{ clipPath: poly }}
            >
