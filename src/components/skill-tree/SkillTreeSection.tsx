...
function NodeBadge({ node, index, selected, flashKey, onSelect }: NodeBadgeProps) {
  ...
  const outlineScales = tier === 3 ? [1.0, 0.91, 0.82] : tier === 2 ? [1.0, 0.89] : [1.0]
  const outlineColors = tier === 3
    ? ['var(--color-accent)', 'var(--color-paper)', 'var(--color-accent)']
    : tier === 2
      ? ['var(--color-accent)', 'var(--color-paper)']
      : ['var(--color-accent-alt)']
  const outlineWidths = tier === 3 ? [2.5, 1.2, 0.8] : tier === 2 ? [2.5, 1.2] : [2.5]
      ...
      >
        ...
        >
          ...
          <svg viewBox="0 0 100 100" className="relative h-full w-full" aria-hidden="true">
            <defs>
              <radialGradient id={`skill-ng-${node.id}`} cx="42%" cy="38%" r="75%">
                <stop offset="0%" stopColor="#ff3b30" />
                <stop offset="70%" stopColor="#e60012" />
                <stop offset="100%" stopColor="#a3000c" />
              </radialGradient>
              <linearGradient id={`skill-ns-${node.id}`} x1="0" y1="0" x2="1" y2="1">
              ...
              <filter id={`skill-noise-${node.id}`}>
                <feTurbulence type="fractalNoise" baseFrequency="0.9" numOctaves="4" seed={index * 7} />
            ...
            <polygon
              points={NODE_POINTS}
              fill={locked ? 'rgba(20,20,20,0.95)' : `url(#skill-ng-${node.id})`}
            ...
            {!locked && <polygon points={NODE_POINTS} fill={`url(#skill-ns-${node.id})`} />}
  ...
  return (
    <svg className="pointer-events-none absolute inset-0" width={w} height={h} aria-hidden="true">
      {SKILL_TREE.edges.map((edge, index) => {
        ...
        const stroke = isFuture ? 'rgba(245,245,240,0.2)' : 'var(--color-accent)'
        ...
        return (
          <g
            key={index}
            style={{
              filter: isFuture ? 'none' : 'drop-shadow(0 0 4px rgba(230,0,18,0.35))',
          ...
          >
            ...
            {!reduced && !isFuture && (
              <g>
                <circle r={6} fill="var(--color-accent)" opacity={0} style={{ filter: 'blur(2px)' }}>
                  <animate attributeName="cx" values={`${a.x};${a.x};${b.x}`} dur="2.8s" begin={`${index * 0.7}s`} repeatCount="indefinite" calcMode="spline" keySplines="0.42 0 0.58 1;0.42 0 0.58 1" />
                  <animate attributeName="cy" values={`${a.y};${cornerY};${cornerY}`} dur="2.8s" begin={`${index * 0.7}s`} repeatCount="indefinite" calcMode="spline" keySplines="0.42 0 0.58 1;0.42 0 0.58 1" />
                  <animate attributeName="opacity" values="0;1;0" keyTimes="0;0.5;1" dur="2.8s" begin={`${index * 0.7}s`} repeatCount="indefinite" calcMode="spline" keySplines="0.42 0 0.58 1;0.42 0 0.58 1" />
                </circle>
                <circle r={2} fill="var(--color-paper)" opacity={0} style={{ filter: 'drop-shadow(0 0 4px rgba(230,0,18,0.9))' }}>
                  <animate attributeName="cx" values={`${a.x};${a.x};${b.x}`} dur="2.8s" begin={`${index * 0.7}s`} repeatCount="indefinite" calcMode="spline" keySplines="0.42 0 0.58 1;0.42 0 0.58 1" />
                  <animate attributeName="cy" values={`${a.y};${cornerY};${cornerY}`} dur="2.8s" begin={`${index * 0.7}s`} repeatCount="indefinite" calcMode="spline" keySplines="0.42 0 0.58 1;0.42 0 0.58 1" />
                  <animate attributeName="opacity" values="0;1;0" keyTimes="0;0.5;1" dur="2.8s" begin={`${index * 0.7}s`} repeatCount="indefinite" calcMode="spline" keySplines="0.42 0 0.58 1;0.42 0 0.58 1" />
                </circle>
...
function DetailPanel({ node }: { node: SkillNode }) {
    ...
    <>
      ...
      {(node.period || node.institution) && (
        <dl className="mt-4 space-y-3">
          {node.period && (
            <div>
              <dt className="font-sans text-[0.65rem] uppercase tracking-[0.18em] text-paper/50">Periodo</dt>
              <dd className="mt-1 font-medium">{node.period}</dd>
            </div>
          )}
          {node.institution && (
            <div>
              <dt className="font-sans text-[0.65rem] uppercase tracking-[0.18em] text-paper/50">Centro</dt>
  ...
  return (
    <div
      ...
      style={{ clipPath: 'polygon(0 0, calc(100% - 1.2rem) 0, 100% 1.2rem, 100% 100%, 0 100%)' }}
    >
...
function CursorDiamond({ containerRef }: { containerRef: React.RefObject<HTMLDivElement | null> }) {
